...
export function DemoDataTable<T>({ columns, rows, getRowKey }: DemoDataTableProps<T>) {
  return (
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-white/10">
            {rows.map((row, index) => (
              <tr key={getRowKey(row, index)} className="bg-black/20">
                {columns.map((column) => (
                  <td key={String(column.key)} className="px-4 py-3 align-top" style={{ color: "var(--color-text)" }}>
