...
export default function SettingsPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      apiClient.get("/settings/webhook-prefs").then((r) => {
        const d = r.data as any;
        setWebhookSettings({ forward_url: d.forward_url||"" });
      })
  ...
  const handleWebhookSave = async (e: React.FormEvent) => { e.preventDefault(); trackEvent("settings_updated",{section:"webhook_prefs"}); try { await apiClient.put("/settings/webhook-prefs",{forward_url:webhookSettings.forward_url}); alert("Webhook preferences updated"); } catch(err:any){ alert("Failed: "+err.message); }};
  ...
  const handleManageSubscription = async () => { trackEvent("subscription_manage_clicked"); if(profile.has_active_subscription){ try{ const{data}=await apiClient.get("/polar/portal") as{data:{portal_url:string}}; window.open(data.portal_url,"_blank"); }catch{alert("Failed to open portal");} }else{ try{ const{data}=await apiClient.post("/polar/checkout",{product_id:product.pricing.polarProductId}) as{data:{checkout_url:string}}; window.open(data.checkout_url,"_blank"); }catch{alert("Failed to start checkout");} }};
      ...
      <section className="p-6 rounded-lg mb-8" style={{backgroundColor:"var(--color-surface)"}}>
        ...
        <form onSubmit={handleWebhookSave} className="space-y-4 max-w-md mb-6">
          <div>
            ...
            <input type="url" value={webhookSettings.forward_url} onChange={(e)=>setWebhookSettings({...webhookSettings,forward_url:e.target.value})} className="input-field w-full" placeholder="https://your-server.com/api/webhooks"/>
        ...
        <div className="pt-6" style={{borderTop:"1px solid var(--color-border)"}}>
          ...
          <button onClick={handleClearHistory} className="px-4 py-2 bg-red-500/10 text-red-500 hover:bg-red-500/20 rounded-md font-medium text-sm transition-colors border border-red-500/20">Clear History</button>
        </div>
      </section>
    </div>
  );
