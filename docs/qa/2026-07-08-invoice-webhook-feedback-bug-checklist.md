...
| IF-31 | Live cron passed aware UTC values through SQLAlchemy columns typed as timezone-naive, causing asyncpg `DataError` during overdue status updates. | All persisted InvoiceFollow timestamps now use `DateTime(timezone=True)` and an idempotent Neon migration converts legacy columns to `TIMESTAMPTZ ... AT TIME ZONE 'UTC'`. | Live cron reproduction and green retest (`reminders: enqueued`); model/migration regression; full suite 331 tests OK; Neon column inspection |
